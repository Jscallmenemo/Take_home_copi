...
db = connector.connect(
    user="bismillahtakehome", password="W6ALd+AV[_ogEu9", host="bismillah.mysql.database.azure.com", port=3306, database="ecommerce"
)
db_config = {
    user="bismillahtakehome", password="W6ALd+AV[_ogEu9", host="bismillah.mysql.database.azure.com", port=3306, database="ecommerce"
}
...
def format_phone_number(phone_number):
    ...
    # Pastikan hanya angka yang diizinkan setelah kode negara
...
def verify_otp():
    ...
    # Verifikasi OTP
    if session.get('otp') == int(input_otp):
        session.pop('otp')  # Hapus OTP setelah diverifikasi
        return jsonify({"success": True, "message": "Nomor HP berhasil diverifikasi."})
    else:
        return jsonify({"success": False, "message": "OTP salah atau kadaluarsa."})
